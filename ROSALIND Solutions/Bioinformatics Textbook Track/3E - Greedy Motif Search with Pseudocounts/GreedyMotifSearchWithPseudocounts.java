 ...
 *  Return: Return: A collection of strings BestMotifs resulting from running GreedyMotifSearch(Dna, k, t) with pseudocounts. If at any step you find more than one Profile-most probable k-mer in a given string, use the one occurring first.
 */
...
public class GreedyMotifSearchWithPseudocounts {
    public static void main( String[] args ) {
        ...
        String[] out = greedyMotifSearchWithPseudocounts(k,t,dna);
        for(int i = 0; i < out.length; ++i) {
            System.out.println(out[i]);
        }
    }
    
    public static String[] greedyMotifSearchWithPseudocounts( int k, int t, String[] dna ) {
        ...
        for(int m = 0; m <= dna[0].length()-k; ++m) {
            ...
            for(int i = 1; i < t; ++i) {
                ...
                double[][] profile = profileFromMotifsWithPseudocounts(prevMotifs);
    ...
    public static double[][] profileFromMotifsWithPseudocounts( String[] motifs ) {
        ...
        for(int i = 0; i < profile.length; ++i) {
            for(int j = 0; j < profile[0].length; ++j) {
                ++profile[i][j];
                profile[i][j] = profile[i][j] / (double)(motifs.length+4);
            }
        }
        return profile;
    }
    ...
    public static int scoreMotifsWithPseudocounts( String[] motifs ) {
        double[][] profile = profileFromMotifsWithPseudocounts(motifs);
        String consensus = "";
