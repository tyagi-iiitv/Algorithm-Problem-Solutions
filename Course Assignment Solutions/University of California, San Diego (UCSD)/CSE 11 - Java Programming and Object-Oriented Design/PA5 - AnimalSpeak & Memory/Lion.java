 ...
 *
 * This file simply creates a Lion object type using the interface AnimalCard.
 * The Lion object has parameters (Image, Location, DrawingCanvas).
 */
...
{
  private static String LION_SPEAK = "Roar"; // What the Cow says
  private        Color highlightColor;       // Highlight Color (Green or Red)
  private        VisibleImage lionPic;       // The Image file (jpg)
  ...
  /*
   * Name:       Lion
   * Purpose:    This constructor defines the Lion object.
   * Parameters: pic:    The image file (lion.jpg in this case)
   *             loc:    The top-left corner of the Lion object
   *             canvas: The canvas that Lion is made on
   * Return:     N/A
   */

  public Lion(Image pic, Location loc, DrawingCanvas canvas)
  {
    lionPic = new VisibleImage( pic, loc, canvas );  // Create image on canvas
  ...
  public String speak()
  {
    return LION_SPEAK; // Return what the animal says
  }
  ...
  {
    return lionPic.contains(point); // Does the animal card contain the point?
  }
...
} // End of public class Lion implements AnimalCard
