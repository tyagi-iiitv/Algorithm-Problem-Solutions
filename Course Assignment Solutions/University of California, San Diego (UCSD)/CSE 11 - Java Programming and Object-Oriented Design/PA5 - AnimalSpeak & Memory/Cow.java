 ...
 *
 * This file simply creates a Cow object type using the interface AnimalCard.
 * The Cow object has parameters (Image, Location, DrawingCanvas).
 */
...
{
  private static String COW_SPEAK = "Moo"; // What the Cow says
  private        Color highlightColor;     // Highlight Color (Green or Red)
  private        VisibleImage cowPic;      // The Image file (jpg)
  ...
  /*
   * Name:       Cow
   * Purpose:    This constructor defines the Cow object.
   * Parameters: pic:    The image file (cow.jpg in this case)
   *             loc:    The top-left corner of the Cow object
   *             canvas: The canvas that Cow is made on
   * Return:     N/A
   */

  public Cow(Image pic, Location loc, DrawingCanvas canvas)
  {
    cowPic = new VisibleImage( pic, loc, canvas );   // Create image on canvas
  ...
  public String speak()
  {
    return COW_SPEAK; // Return what the animal says
  }
  ...
  {
    return cowPic.contains(point); // Does the animal card contain the point?
  }
  ...
  {
    return border.getColor(); // Return the highlight color
  }
...
} // End of public class Cow implements AnimalCard
