 ...
 *
 * This file simply creates a Lamb object type using the interface AnimalCard.
 * The Lamb object has parameters (Image, Location, DrawingCanvas).
 */
...
{
  private static String LAMB_SPEAK = "Baaa"; // What the Cow says
  private        Color highlightColor;       // Highlight Color (Green or Red)
  private        VisibleImage lambPic;       // The Image file (jpg)
  ...
  /*
   * Name:       Lamb
   * Purpose:    This constructor defines the Lamb object.
   * Parameters: pic:    The image file (lamb.jpg in this case)
   *             loc:    The top-left corner of the Lamb object
   *             canvas: The canvas that Lamb is made on
   * Return:     N/A
   */

  public Lamb(Image pic, Location loc, DrawingCanvas canvas)
  {
    lambPic = new VisibleImage( pic, loc, canvas );  // Create image on canvas
  ...
  public String speak()
  {
    return LAMB_SPEAK; // Return what the animal says
  }
  ...
  {
    return lambPic.contains(point); // Does the animal card contain the point?
  }
...
} // End of public class Lamb implements AnimalCard
